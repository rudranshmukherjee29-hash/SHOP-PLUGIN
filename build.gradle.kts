...
tasks.named<ShadowJar>("shadowJar") {
    ...
    // NOTE: org.sqlite is intentionally NOT relocated. sqlite-jdbc's NativeDB
    // extracts and loads a native library at runtime, and that native binary
    // has org.sqlite's original class/method names baked in at compile time.
    // Relocating the Java package breaks the JNI binding between the native
    // library and the relocated class, producing UnsatisfiedLinkError at
    // runtime even though the build succeeds. HikariCP and the MySQL driver
    // are pure Java and safe to relocate.
